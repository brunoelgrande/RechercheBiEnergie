...
if submit:
    equip_prop = [cond_prop.upper(), evap_prop.upper(), fournaise_prop.upper()]
    ...
    # Si au moins au matches sur un des types d'équipements
        ...
        df_cond = df_CEE.iloc[df_matches.query("Condenseur==True")['index']]
        df_evap = df_CEE.iloc[df_matches.query("Evaporateur==True")['index']]

        # Un TRIO existe
        if (len(df_trio) > 0):
            c2.header("Vérification des TRIO  :white_check_mark:")
            # modifier pour un plus beau tableau + éliminer index
            c2.dataframe(df_trio.sort_values('AHRI'), use_container_width=True)
        else:
        ...
        if (len(df_duo) > 0):
            ...
            if (len(df_matches.query('Condenseur == True')) > 0):  # Au moins 1 match sur un condenseur
                verif_duo_exp = c2.expander(
                ...
                evap = pd.DataFrame(df_cond['Evaporateur'].unique().tolist())
            ...
            if (len(df_matches.query('Evaporateur == True')) > 0):
                ...
                verif_duo_exp.dataframe(condenseur, use_container_width=True)
                verif_duo_exp.write("")
                verif_duo_exp.write(
                    '_Attention : bien valider la sélection complète_')
